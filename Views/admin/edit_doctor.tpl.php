...
// Получаем статус и ID клиники текущего авторизованного пользователя
        ...
        <div class="row justify-content-center">
            <div class="col-10 col-md-8 col-lg-6">
                ...
                <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
                    ...
                    <!-- НАЧАЛО ФОРМЫ -->
                    <form action="/clinic/admin/admin/doctor/update" method="POST" class="needs-validation" novalidate>
                        <div class="card-body p-4">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="has-validation">
                                    ...
                                    <?php else: ?>
                                        <!-- Если вошел Главный Администратор (status = 0) — ему по-прежнему доступны все города [image_m2x-vD.png] -->
                                        <select class="form-select bg-light py-2.5 px-3 border" id="doctor_clinic" name="clinic_id" required>
                                            <option value="" disabled>Выберите клинику из списка...</option>
                                            <?php foreach ($clinics as $clinic): ?>
                                                <option value="<?= htmlspecialchars($clinic->id) ?>" <?= $clinic->id == $vet['clinic_id'] ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($clinic->name) ?> (<?= htmlspecialchars($clinic->city) ?>)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
